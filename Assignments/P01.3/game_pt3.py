...
class Player(pygame.sprite.Sprite):
    def __init__(self,screen,scene,imgUp,imgDown,imgLeft,imgRight):
        ...
        self.left = False
        self.right = False
        self.colis = False #Shows alive
        self.money = []
        for j in range(0, 8):
            self.money.append(r"C:\Users\Landen\OneDrive\Desktop\HelperCode\money" + str(j)+".png")
            j = j+1
    ...
    def Crash(self):
        if(self.num == 0):
            if(self.index == 8):
                return True
            self.image = pygame.image.load(self.money[self.index])
